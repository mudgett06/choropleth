...
import { subrange, mode } from "../math";
import { sort_by, groupBy, toProperCase } from "../utility";

String.prototype.toProperCase = toProperCase;

async function getGeoGuess(fileName, propertyNames, uniqueColumnData) {
  let guess = propertyNames
    .map((propName) => guessGeography(propName))
    .map((x, i, arr) => (arr.indexOf(x) == i ? x : null));
  if (guess.every((x) => !x)) {
    await axios.get("/api/states/names").then((res) => {
      const states = data;
      let bestMatchLength = 0;
      guess = uniqueColumnData.reduce((guess, col, i) => {
        const stateMatches = col.filter(
          (x) => states.indexOf(x.toString()) > -1
        );
        if (stateMatches.length > bestMatchLength) {
          bestMatchLength = stateMatches.length;
          return [...guess.slice(0, i), "state", ...guess.slice(i + 1)];
        } else {
          return guess;
        }
      }, guess);
  ...
  if (guess.every((x) => !x)) {
    guess = [guessGeography(fileName), ...guess.slice(1)];
  }
  return guess;
}
